/**
    ...
    Blocking communications are being used.
  **/
...
int main(int argc, char **argv)
{
  if (argc != 4)
  {
    ...
    printf("and the txt file with the labels...\n");
    exit(1);
  }
  ...
  **/
  ...
  for (i=0; i<(numtasks-1); i++)
  {
    if (rank % 2)
    {
      MPI_Send(&(new_obs[0][0]), chunk*dimensions, MPI_DOUBLE, next, tag, MPI_COMM_WORLD);
      MPI_Recv(&(temp[0][0]), chunk*dimensions, MPI_DOUBLE, prev, tag, MPI_COMM_WORLD, &stats[0]);
    }
    else
    {
      MPI_Recv(&(temp[0][0]), chunk*dimensions, MPI_DOUBLE, prev, tag, MPI_COMM_WORLD, &stats[1]);
      MPI_Send(&(new_obs[0][0]), chunk*dimensions, MPI_DOUBLE, next, tag, MPI_COMM_WORLD);
    }
  ...
  **/
  ...
  for (i=0; i<(numtasks-1); i++)
  {
    if (rank % 2)
    {
      MPI_Send(&(new_labels[0]), chunk, MPI_INT, next, tag, MPI_COMM_WORLD);
      MPI_Recv(&(temp_labels[0]), chunk, MPI_INT, prev, tag, MPI_COMM_WORLD, &stats[0]);
    }
    else
    {
      MPI_Recv(&(temp_labels[0]), chunk, MPI_INT, prev, tag, MPI_COMM_WORLD, &stats[1]);
      MPI_Send(&(new_labels[0]), chunk, MPI_INT, next, tag, MPI_COMM_WORLD);
    }
